...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        self.upload_photo_button = tk.Button(self.root, text="Upload Photo", command=self.upload_photo, font=("Arial", 12))
        self.upload_photo_button.pack(pady=5)

        self.process_photo_button = tk.Button(self.root, text="Process Photo", command=self.process_photo, font=("Arial", 12))
        self.process_photo_button.pack(pady=5)
        self.process_photo_button.config(state=tk.DISABLED)

        # Video upload and processing
        self.upload_video_button = tk.Button(self.root, text="Upload Video", command=self.upload_video, font=("Arial", 12))
        self.upload_video_button.pack(pady=5)

        self.start_video_button = tk.Button(self.root, text="Start Video Detection", command=self.start_video_detection, font=("Arial", 12))
        self.start_video_button.pack(pady=5)
        self.start_video_button.config(state=tk.DISABLED)

        # Webcam processing
        self.start_webcam_button = tk.Button(self.root, text="Start Webcam", command=self.start_webcam, font=("Arial", 12))
        self.start_webcam_button.pack(pady=5)

        # Stop detection (for both video and webcam)
        self.stop_button = tk.Button(self.root, text="Stop Detection", command=self.stop_detection, font=("Arial", 12))
        self.stop_button.pack(pady=5)
        self.stop_button.config(state=tk.DISABLED)

        # Canvas for displaying photo, video, or webcam feed
    ...
    def start_webcam(self):
        """Start webcam feed for real-time face recognition."""
        self.is_webcam = True
    ...
    def stop_detection(self):
        """Stop the video or webcam processing."""
        self.is_running = False
        self.start_video_button.config(state=tk.NORMAL)
    ...
    def process_video(self):
        """Process the video or webcam feed and perform face recognition."""
        ...
        self.start_webcam_button.config(state=tk.NORMAL)
        self.stop_button.config(state=tk.DISABLED)
        self.status_label.config(text=f"Status: {'Webcam' if self.is_webcam else 'Video'} Stopped. Output saved to {self.output_video_path}")

    def display_frame(self, frame):
        """Display the frame (photo, video, or webcam) on the Tkinter canvas."""
