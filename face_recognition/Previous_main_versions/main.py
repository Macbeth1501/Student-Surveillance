...
class FaceRecognitionApp:
    def __init__(self, root):
        ...
        self.start_video_button = tk.Button(self.root, text="Start Video Detection", command=self.start_detection, font=("Arial", 12))
        self.start_video_button.pack(pady=5)
        self.start_video_button.config(state=tk.DISABLED)

        self.stop_video_button = tk.Button(self.root, text="Stop Video Detection", command=self.stop_detection, font=("Arial", 12))
        self.stop_video_button.pack(pady=5)
        self.stop_video_button.config(state=tk.DISABLED)

        # Canvas for displaying photo or video
    ...
    def process_photo(self):
        ...
        output_photo_path = "output_photo.jpg"
        cv2.imwrite(output_photo_path, frame)

        # Display the processed photo on the canvas
        self.display_frame(frame)
        self.status_label.config(text=f"Status: Photo Processed. Saved to {output_photo_path}")
    ...
    def process_video(self):
        """Process the video and perform face recognition."""
        ...
        if not self.cap.isOpened():
            ...
            self.start_video_button.config(state=tk.NORMAL)
            self.stop_video_button.config(state=tk.DISABLED)
            self.status_label.config(text="Status: Error - Could not open video")
            return
        ...
        self.start_video_button.config(state=tk.NORMAL)
        self.stop_video_button.config(state=tk.DISABLED)
        self.status_label.config(text=f"Status: Video Completed. Output saved to {self.output_video_path}")

    def display_frame(self, frame):
        """Display the frame (photo or video) on the Tkinter canvas."""
